...
_NARRATE_PROMPT = """คุณคือผู้ช่วยวิเคราะห์ข้อมูลโรงงาน ตอบเป็นภาษาไทย

คำถามของผู้ใช้: {question}

ผลการคำนวณจากฐานข้อมูลจริง (คำนวณโดยระบบแล้ว):
{result}

เขียนคำตอบสั้น กระชับ 1-3 ประโยค
กติกาเด็ดขาด:
...
- ไม่ต้องขึ้นต้นด้วยคำทักทาย ไม่ต้องสรุปซ้ำท้ายประโยค
...
def ask(question, provider, api_key, model):
    ...
    if not orders:
        return {"answer": "ยังไม่มีข้อมูลใบสั่งผลิตในระบบ", "kind": "empty",
                "rows": [], "cached": False}
    ...
    if intent == "unsupported":
        payload = {"answer": "คำถามนี้อยู่นอกขอบเขตข้อมูลใบสั่งผลิต "
                             "ลองถามเกี่ยวกับปริมาณการใช้วัตถุดิบ ยอดผลิต "
                             "หรือการเปรียบเทียบระหว่างช่วงเวลาดูครับ",
    ...
    if result.get("not_found"):
        payload = {"answer": f"ไม่พบ \"{result['not_found']}\" ในข้อมูลที่มี "
                             "ลองตรวจสอบชื่อวัตถุดิบหรือผลิตภัณฑ์อีกครั้งครับ",
